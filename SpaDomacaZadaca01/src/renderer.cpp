...
void Renderer::render(MeshObject& object) {
	...
	Mat4 screen_projection_matrix = Renderer::create_screen_projection_matrix((float)object.window->getSize().x, (float)object.window->getSize().y);
	...
	for (Tri& tri : visible_tris) {
		...
		projected.normalize_to_viewport(object.window->getSize().x, object.window->getSize().y);
