...
const std::string WINDOW_TITLE = "SFML Software Renderer";

const std::string DEFAULT_MODELS_PATH   = "resources/models/";
const std::string DEFAULT_TEXTURES_PATH = "resources/textures/";
...
int main(int argc, char* argv[]) {
	...
	sf::View screen_view = window.getDefaultView();
		...
		while (window.pollEvent(event)) {
			switch (event.type) {
				case sf::Event::Closed:
					window.close();
					break;
				case sf::Event::Resized: // Try to fix stretching on resize...
					screen_view.setSize(event.size.width, event.size.height);
					screen_view.setCenter(event.size.width / 2.f, event.size.height / 2.f);
					break;
